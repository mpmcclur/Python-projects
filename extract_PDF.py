...
"""This program is designed to import Thorlabs' paystub PDF files from Paychex, extract specific information,
   ...
   this program pulled the correct information 100% of the time for the new paystub format. For the old paystub format,
   correct information was pulled ~50% of the time or program failure, due to the significant variance in the format
   from paycheck to paycheck.
   Thus, this program is recommended only from step 3, onward, with the new paystub style.
...
def extractmultiple():
    ...
    for file in pdfs:
        ...
        dflist.append(df1)
    data = pd.DataFrame(dflist,columns=['Date','Net Pay PER','Net Pay YTD','Tot Hours','Fed Tax','NJ Tax','401K Cont','401K Match','NJ Disability','NJ Unemploy','NJ EE Work Dev','CAF Dental','CAF Medical'])
    data.to_csv('PayStub_Data.csv', header=True, mode='a',index=False)
extractmultiple()
